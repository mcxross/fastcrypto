...
pub fn seal_ibe_encrypt(
    ...
    if public_keys.len() != shares.len()
        || public_keys.len() != object_ids.len()
        || public_keys.len() != indices.len()
    {
        return Err(FastCryptoFfiError::InvalidInput);
    }
    ...
    for i in 0..public_keys.len() {
        let mut pk_affine = blst_p2_affine::default();
        unsafe {
            if blst_p2_uncompress(&mut pk_affine, public_keys[i].as_ptr())
...
pub fn seal_verify_user_secret_key(
    ...
    unsafe { Ok(blst_fp12_is_equal(&lhs, &rhs)) }
}
...
#[uniffi::export]
pub fn seal_extract_usk(
    master_sk_bytes: Vec<u8>,
    id: Vec<u8>,
) -> Result<Vec<u8>, FastCryptoFfiError> {
...
pub fn seal_aes_gcm_encrypt(
    ...
    if nonce.len() != 12 {
        return Err(FastCryptoFfiError::InputLengthWrong(12));
    }
